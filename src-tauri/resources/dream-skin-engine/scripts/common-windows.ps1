...
function Install-DreamSkinRuntimeEngine {
  ...
  foreach ($directoryName in @('assets', 'runtime', 'scripts')) {
  ...
  try {
    Copy-Item -LiteralPath (Join-Path $sourceRoot 'ENGINE-SOURCE.json') -Destination $stagingRoot `
    ...
    $sourceFiles = @(
      Get-Item -LiteralPath (Join-Path $sourceRoot 'ENGINE-SOURCE.json') -Force -ErrorAction Stop
      Get-ChildItem -LiteralPath (Join-Path $sourceRoot 'assets'), (Join-Path $sourceRoot 'runtime'), `
        (Join-Path $sourceRoot 'scripts') `
        -Recurse -File -Force -ErrorAction Stop
    )
    $stagedFiles = @(
      Get-Item -LiteralPath (Join-Path $stagingRoot 'ENGINE-SOURCE.json') -Force -ErrorAction Stop
      Get-ChildItem -LiteralPath (Join-Path $stagingRoot 'assets'), (Join-Path $stagingRoot 'runtime'), `
...
function Get-DreamSkinNodeRuntime {
  ...
  if (-not $PreferredPath -and $script:DreamSkinPreferredNodePath) {
    $PreferredPath = "$script:DreamSkinPreferredNodePath"
  }
  if ($PreferredPath) { $candidates.Add([System.IO.Path]::GetFullPath($PreferredPath)) }
  $privateNode = Join-Path (Split-Path -Parent $PSScriptRoot) 'runtime\node.exe'
  $candidates.Add($privateNode)

  $commands = @(
    (Get-Command node.exe -All -ErrorAction SilentlyContinue),
    (Get-Command node -All -ErrorAction SilentlyContinue)
  )
  foreach ($command in $commands) {
    if ($null -ne $command -and $command.Source) { $candidates.Add("$($command.Source)") }
  }
  ...
  foreach ($root in $registryRoots) {
    ...
    foreach ($name in @('Path', 'NODE_BINARY', 'NVM_HOME', 'NVM_SYMLINK', 'FNM_DIR',
        'FNM_MULTISHELL_PATH', 'NODE_HOME', 'NODEJS_HOME', 'VOLTA_HOME')) {
      $value = $properties.$name
      if ($value) {
        if (-not $registryValues.ContainsKey($name)) { $registryValues[$name] = @() }
        $registryValues[$name] += "$value"
      }
    }
  }

  foreach ($pathValue in @($env:PATH) + @($registryValues['Path'])) {
    if (-not $pathValue) { continue }
    foreach ($directory in ([Environment]::ExpandEnvironmentVariables("$pathValue") -split ';')) {
      if ($directory) { $candidates.Add((Join-Path $directory 'node.exe')) }
    }
  }

  $addVersionManagerRoot = {
    param([string]$Root)
    if (-not $Root) { return }
    $expandedRoot = [Environment]::ExpandEnvironmentVariables($Root)
    if (-not (Test-Path -LiteralPath $expandedRoot -PathType Container)) { return }
    $candidates.Add((Join-Path $expandedRoot 'node.exe'))
    foreach ($versionDirectory in @(Get-ChildItem -LiteralPath $expandedRoot -Directory -ErrorAction SilentlyContinue)) {
      $candidates.Add((Join-Path $versionDirectory.FullName 'node.exe'))
      $candidates.Add((Join-Path $versionDirectory.FullName 'installation\node.exe'))
    }
    $nodeVersions = Join-Path $expandedRoot 'node-versions'
    foreach ($versionDirectory in @(Get-ChildItem -LiteralPath $nodeVersions -Directory -ErrorAction SilentlyContinue)) {
      $candidates.Add((Join-Path $versionDirectory.FullName 'installation\node.exe'))
    }
  }

  foreach ($name in @('NODE_BINARY', 'NVM_HOME', 'NVM_SYMLINK', 'FNM_DIR',
      'FNM_MULTISHELL_PATH', 'NODE_HOME', 'NODEJS_HOME', 'VOLTA_HOME')) {
    $values = @([Environment]::GetEnvironmentVariable($name, 'Process')) + @($registryValues[$name])
    foreach ($value in $values) {
      if (-not $value) { continue }
      $expanded = [Environment]::ExpandEnvironmentVariables("$value")
      switch ($name) {
        'NODE_BINARY' { $candidates.Add($expanded) }
        'NVM_HOME' { & $addVersionManagerRoot $expanded }
        'FNM_DIR' { & $addVersionManagerRoot $expanded }
        'VOLTA_HOME' { $candidates.Add((Join-Path $expanded 'bin\node.exe')) }
        default { $candidates.Add((Join-Path $expanded 'node.exe')) }
      }
    }
  }
  ...
  if ($env:LOCALAPPDATA) { & $addVersionManagerRoot (Join-Path $env:LOCALAPPDATA 'fnm') }
  if ($env:LOCALAPPDATA) { & $addVersionManagerRoot (Join-Path $env:LOCALAPPDATA 'nvm') }
  if ($env:USERPROFILE) { & $addVersionManagerRoot (Join-Path $env:USERPROFILE '.fnm') }
  if ($env:USERPROFILE) { & $addVersionManagerRoot (Join-Path $env:USERPROFILE 'AppData\Roaming\nvm') }

  $seen = @{}
  $unsupported = $null
  foreach ($candidate in $candidates) {
    $key = $candidate.ToLowerInvariant()
    if ($seen.ContainsKey($key) -or -not (Test-Path -LiteralPath $candidate -PathType Leaf)) { continue }
    $seen[$key] = $true
    ...
    if ($pathProbe.ExitCode -ne 0 -or -not $runtimePath -or -not (Test-Path -LiteralPath $runtimePath -PathType Leaf)) { continue }
    $major = 0
    if (-not [int]::TryParse(($version -split '\.')[0], [ref]$major)) { continue }
    $runtime = [pscustomobject]@{ Path = $runtimePath; Version = $version; Major = $major }
    if ($major -ge $MinimumMajor) { return $runtime }
    if ($null -eq $unsupported) { $unsupported = $runtime }
  }
  if ($null -ne $unsupported) {
    throw "Node.js $MinimumMajor or newer is required; found $($unsupported.Version) at $($unsupported.Path)."
  }
  throw "Node.js $MinimumMajor or newer is required and was not found in the bundled runtime, live Windows environment, or common installation locations."
}
...
function Wait-DreamSkinRecordedInjectorExit {
  param([Parameter(Mandatory = $true)][object]$State, [int]$TimeoutSeconds = 15)
  $processId = [int]$State.injectorPid
  $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
  while ((Get-Date) -lt $deadline) {
    $process = Get-CimInstance Win32_Process -Filter "ProcessId = $processId" -ErrorAction SilentlyContinue
    if ($null -eq $process) { return $true }
    if (-not (Test-DreamSkinRecordedInjectorIdentity -State $State -Process $process)) { return $true }
    Start-Sleep -Milliseconds 250
  }
  $process = Get-CimInstance Win32_Process -Filter "ProcessId = $processId" -ErrorAction SilentlyContinue
  return $null -eq $process -or -not (Test-DreamSkinRecordedInjectorIdentity -State $State -Process $process)
}
...
function Stop-DreamSkinRecordedInjector {
  ...
  if (-not (Wait-DreamSkinRecordedInjectorExit -State $State -TimeoutSeconds 15)) {
