...
function Initialize-DreamSkinThemeStore {
  ...
  $assetImage = Join-Path $assetRoot 'portal-hero.png'
  ...
  if (-not (Test-Path -LiteralPath $activeTheme -PathType Leaf)) {
    ...
    Assert-DreamSkinNoReparseComponents -Path (Join-Path $paths.Active 'portal-hero.png')
    $activeImage = Join-Path $paths.Active 'portal-hero.png'
    Copy-Item -LiteralPath (Join-Path $assetRoot 'portal-hero.png') `
    ...
    $imageArchive = Join-Path $paths.Images 'portal-hero.png'
  ...
  $presetDirectory = Join-Path $paths.Saved 'preset-dream-portal'
  ...
  if (-not (Test-Path -LiteralPath $presetTheme -PathType Leaf)) {
    ...
    $presetImage = Join-Path $presetDirectory 'portal-hero.png'
