...
function Test-DreamSkinVerificationWaitingForRenderer {
  [OutputType([bool])]
  param(
    [AllowEmptyString()]
  ...
  if ($VerificationText -match 'No verified Codex renderer on 127\.0\.0\.1:') {
    return $true
  }
  ...
  try {
    ...
    $hasTransientRenderer = $false
    foreach ($target in $targets) {
      $expectedVersion = [string]$target.result.expectedVersion
      $isTransientRenderer = $target.result.installed -eq $false -and
        -not [string]::IsNullOrWhiteSpace($expectedVersion)
      if ($isTransientRenderer) {
        $hasTransientRenderer = $true
      } elseif ($target.result.pass -ne $true) {
        return $false
      }
    }
    return $hasTransientRenderer
  } catch {
    return $false
  }
...
try {
  ...
  $script:DreamSkinPreferredNodePath = $NodePath
  ...
  try {
    ...
    $verify = Invoke-DreamSkinNative -FilePath $node.Path -ArgumentList @(
      ...
      '--browser-id', $cdpIdentity.BrowserId, '--timeout-ms', '6000')
    $verifyText = ($verify.Output -join "`r`n")
    Write-DreamSkinUtf8FileAtomically -Path $VerifyPath -Content ($verifyText + "`r`n")
    if ($verify.ExitCode -ne 0) {
      $rendererPending = Test-DreamSkinVerificationWaitingForRenderer -VerificationText $verifyText
      $rendererPending = $rendererPending -and $null -ne $daemon -and -not $daemon.HasExited
      if ($rendererPending) {
        Write-Warning 'Codex is still loading its main interface; the verified watcher will inject as soon as the renderer is ready.'
      } else {
