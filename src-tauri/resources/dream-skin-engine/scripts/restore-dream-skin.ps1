...
try {
  ...
  $restoreBackupPath = $backup
  $restoreBackupIsActive = Test-Path -LiteralPath $backup
  if ($RestoreBaseTheme -and -not $restoreBackupIsActive) {
    $archivedBackup = Get-ChildItem -LiteralPath $StateRoot -Filter 'config.restored-*.toml' -File `
      -ErrorAction SilentlyContinue | Sort-Object Name -Descending | Select-Object -First 1
    if ($null -ne $archivedBackup) {
      $restoreBackupPath = $archivedBackup.FullName
      Write-Warning "Using archived pre-install config backup: $restoreBackupPath"
    } else {
      $restoreBackupPath = $null
      Write-Warning 'No pre-install config backup is available; continuing because the official appearance may already be restored.'
    }
  }
  ...
  } elseif ($RestoreBaseTheme -and $null -ne $restoreBackupPath) {
    $null = Read-DreamSkinUtf8File -Path $restoreBackupPath
  ...
  try {
    ...
    if ($shouldCloseCodex) {
      ...
      if ($portOwnedByCodex -and -not (Wait-DreamSkinPortAvailable -Port $Port -TimeoutSeconds 15)) {
        throw "Port $Port did not become available within 15 seconds after Codex closed; state and configuration were preserved."
      }
    }
    ...
    if ($RecoverConfigBackup) {
      ...
      Restore-DreamSkinBaseTheme -ConfigPath $config -BackupPath $restoreBackupPath
    }
    if ($RecoverConfigBackup -or ($RestoreBaseTheme -and $restoreBackupIsActive)) {
    ...
    if (Test-Path -LiteralPath $PausedPath) {
      Remove-Item -LiteralPath $PausedPath -Force -ErrorAction Stop
    }
    if (Test-Path -LiteralPath $StatePath) {
      throw 'Dream Skin runtime state still exists after restore.'
    }
    if (Test-Path -LiteralPath $PausedPath) {
      throw 'Dream Skin pause marker still exists after restore.'
    }
    if ($Uninstall) {
